...
final class PresentCardTransitionDriver {
    ...
    init(params: PresentCardAnimator.Params, transitionContext: UIViewControllerContextTransitioning, baseAnimator: UIViewPropertyAnimator) {
        ...
        let screens: (home: FeedVC, cardDetail: PopUpDetailVC) = (
            ctx.viewController(forKey: .from)! as! FeedVC,
            ctx.viewController(forKey: .to)! as! PopUpDetailVC
        )
        ...
        let topTemporaryFix = screens.cardDetail.cardView.topAnchor.constraint(equalTo: cardDetailView.topAnchor, constant: 0)
